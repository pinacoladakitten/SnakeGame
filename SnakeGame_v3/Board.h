...
class Board {
private:
    int frameMax=100000,         // The number of iterations it takes for the frame to update while running
        frame=0,                // The iterator (increases by 1 for each loop)
        rows, cols, playerNum=1;  // The number of rows and columns the board has
    
    bool bIsRun = true;         // A toggle to keep the game running
    
    SnakePlayer** players;       // A pointer to the player object
